...
║  {Fore.YELLOW}╚═╝     ╚══════╝╚═╝╚═╝  ╚═╝ ╚═════╝     {Fore.CYAN}║
║                                                      ║
║           {Fore.MAGENTA} PsikoLOG Call Spam v1.0{Fore.CYAN}              ║
╚══════════════════════════════════════════════════╝
            """)
        
        print(f"{Fore.RED}{Style.BRIGHT}⚡ {Fore.WHITE}Geliştirici: {Fore.CYAN}PsikoLOG | crackermain.net")
...
class AramaMotoru:
    ...
    def _ayarlar_menu(self):
        while True:
            ...
            print(f"║  {Fore.YELLOW}[2] {Fore.WHITE}Animasyon Hizi: {Fore.GREEN}{self.ayarlar['animasyon_hizi']}")
            print(f"║  {Fore.YELLOW}[3] {Fore.WHITE}Otomatik Cevap: {Fore.GREEN if self.ayarlar['otomatik_cevap'] else Fore.RED}{'Acik' if self.ayarlar['otomatik_cevap'] else 'Kapali'}")
            print(f"║  {Fore.YELLOW}[4] {Fore.WHITE}Debug Modu: {Fore.GREEN if self.ayarlar['debug_modu'] else Fore.RED}{'Acik' if self.ayarlar['debug_modu'] else 'Kapali'}")
            print(f"║  {Fore.YELLOW}[5] {Fore.WHITE}Maksimum Tekrar: {Fore.GREEN}{self.ayarlar['maksimum_tekrar']}")
            print(f"║  {Fore.YELLOW}[6] {Fore.WHITE}Ana Menuye Don")
            print(f"║                                                      ║")
            print(f"{Fore.CYAN}╚══════════════════════════════════════════════════╝")
            
            secim = input(f"\n{Fore.YELLOW}Seciminiz (1-6): {Fore.WHITE}")
            
            if secim == "1":
                try:
                    yeni_sure = int(input(f"{Fore.WHITE}Yeni bekleme suresi (saniye): "))
                    if yeni_sure < 0:
                        print(f"{Fore.RED}Negatif deger olamaz!")
                    else:
                        self.ayarlar['bekleme_suresi'] = yeni_sure
                        self.rate_limiter.bekleme_suresi_degistir(yeni_sure)
                        print(f"{Fore.GREEN}Bekleme suresi guncellendi!")
                except ValueError:
                    print(f"{Fore.RED}Gecersiz deger!")
                time.sleep(1)
                
            elif secim == "2":
                try:
                    yeni_hiz = float(input(f"{Fore.WHITE}Yeni animasyon hizi (0.01-0.1): "))
                    if 0.01 <= yeni_hiz <= 0.1:
                        self.ayarlar['animasyon_hizi'] = yeni_hiz
                        print(f"{Fore.GREEN}Animasyon hizi guncellendi!")
                    else:
                        print(f"{Fore.RED}Deger 0.01-0.1 arasinda olmali!")
                except ValueError:
                    print(f"{Fore.RED}Gecersiz deger!")
                time.sleep(1)
                
            elif secim == "3":
                self.ayarlar['otomatik_cevap'] = not self.ayarlar['otomatik_cevap']
                print(f"{Fore.GREEN}Otomatik cevap: {'Acik' if self.ayarlar['otomatik_cevap'] else 'Kapali'}")
                time.sleep(1)
                
            elif secim == "4":
                self.ayarlar['debug_modu'] = not self.ayarlar['debug_modu']
                print(f"{Fore.GREEN}Debug modu: {'Acik' if self.ayarlar['debug_modu'] else 'Kapali'}")
                time.sleep(1)
                
            elif secim == "5":
                try:
                    yeni_tekrar = int(input(f"{Fore.WHITE}Maksimum tekrar sayisi (1-10): "))
                    if 1 <= yeni_tekrar <= 10:
                        self.ayarlar['maksimum_tekrar'] = yeni_tekrar
                        print(f"{Fore.GREEN}Maksimum tekrar guncellendi!")
                    else:
                        print(f"{Fore.RED}Deger 1-10 arasinda olmali!")
                except ValueError:
                    print(f"{Fore.RED}Gecersiz deger!")
                time.sleep(1)
    ...
    def _istatistik_goster(self):
        ...
        calisma_suresi = datetime.now() - self.genel_istatistikler['baslangic_zamani']
        saat = calisma_suresi.seconds // 3600
        dakika = (calisma_suresi.seconds % 3600) // 60
        saniye = calisma_suresi.seconds % 60
        
        print(f"║  {Fore.YELLOW}Calisma Suresi: {Fore.WHITE}{saat:02d}:{dakika:02d}:{saniye:02d}")
        print(f"║                                                      ║")
        print(f"║  {Fore.CYAN}Arama Istatistikleri:")
        print(f"║     Toplam Arama: {Fore.WHITE}{self.genel_istatistikler['toplam_arama']}")
        ...
        if self.genel_istatistikler['toplam_arama'] > 0:
            basari_orani = (self.genel_istatistikler['basarili_arama'] / self.genel_istatistikler['toplam_arama']) * 100
            print(f"║     Basari Orani: {Fore.GREEN}%{basari_orani:.1f}")
        
        print(f"║                                                      ║")
        print(f"║  {Fore.BLUE}API Istatistikleri:")
        print(f"║     Toplam Istek: {Fore.WHITE}{self.genel_istatistikler['api_istekleri']}")
        
        rate_istatistik = self.rate_limiter.istatistik_al()
        if rate_istatistik:
            print(f"║  {Fore.MAGENTA}Rate Limit Istatistikleri:")
            print(f"║     Izin Verilen: {Fore.GREEN}{rate_istatistik.get('izin_verilen', 0)}")
            print(f"║     Reddedilen: {Fore.RED}{rate_istatistik.get('reddedilen', 0)}")
        
        print(f"║                                                      ║")
        print(f"║  {Fore.YELLOW}Aktif Ayarlar:")
        print(f"║     Bekleme Suresi: {Fore.WHITE}{self.ayarlar['bekleme_suresi']}s")
        print(f"║     Debug Modu: {Fore.GREEN if self.ayarlar['debug_modu'] else Fore.RED}{'Acik' if self.ayarlar['debug_modu'] else 'Kapali'}")
        print(f"║     Otomatik Cevap: {Fore.GREEN if self.ayarlar['otomatik_cevap'] else Fore.RED}{'Acik' if self.ayarlar['otomatik_cevap'] else 'Kapali'}")
        print(f"║                                                      ║")
        print(f"{Fore.CYAN}╚══════════════════════════════════════════════════╝")
    ...
    def _cikis(self):
        self.ui.banner_goster()
        self.ui.animasyonlu_yaz("Gule gule! Program kapatiliyor...", 0.03, Fore.MAGENTA)
        self.aktif = False
        sys.exit(0)
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n{Fore.YELLOW}Program kullanici tarafindan sonlandirildi.")
    except Exception as e:
